...
// EnergyDepositionHistograms AnalyzeHistogramsFromSimulation(const DecayDynamics& decayDynamicsInstance, int numberIterations, int caseNum);
...
EnergyDepositionHistograms AnalyzeHistogramsFromSimulation(DecayDynamics decayDynamicsInstance, int numberIterations, int caseNum)
{
    ...
    auto MakeHistogramOneIteration = [&](TFile *inputFile_solution, TFile *inputFile_membrane, TFile *inputFile_cytoplasm, int itNum)
    {
        ...
        while(myReaderSolutionSim.Next())
        {
            ...
            // for(int i=0; i<particleTypeSolutionSim.GetSize(); i++)
            // {
            //     if(particleTypeSolutionSim[i]==1000020040)
            //     {
            //         std::cout << particleTypeSolutionSim[i] << std::endl;
            //     }
            // }
            ...
            if(firstInteractionInSolution1hTo2h)
            {
                ...
                for(int i=0; i<energyDepsSolutionSim.GetSize(); i++)
                {
                    ...
                    if(particleTypeSolutionSim[i]==4)
                    {
                        // std::cout << "Found alpha in " <<  volumeTypesSolutionSim[i] << std::endl;
                        num++;
                    }
        ...
        std::cout << "Found " << num << " alphas in solution" << std::endl;
        num = 0;
        //------------------–----------
            ...
            while(myReaderMembraneSim.Next())
            {
                ...
                if(firstInteractionInMembrane1hTo26h)
                {
                    ...
                    for(int i=0; i<energyDepsMembraneSim.GetSize(); i++)
                    {
                        ...
                        if(particleTypeMembraneSim[i]==4)
                        {
                            // std::cout << "Found alpha in " <<  volumeTypesSolutionSim[i] << std::endl;
                            num++;
                        }
        ...
        std::cout << "Found " << num << " alphas in membrane" << std::endl;
    ...
    std::string filepathSimulationOutput = "/Users/idarosenqvist/Desktop/Academics/Codes/AlphaTherapyMaster/GEANT4Simulations/OutputDNA/";


    // std::string filepathSimulationOutput = "/Volumes/SamsungT7/G4SEMOut/";
    // std::string filepathSimulationOutput = "/Volumes/SamsungT7/G4DNAOut/";
    ...
    for(int i=startIt; i<stopIt; i++)
    {
        ...
        // filepathSolutionIteration_i = filepathSimulationOutput + "Test.root";
        // std::string filepathMembraneIteration_i = filepathSimulationOutput + "Test.root";
        // std::string filepathCytoplasmIteration_i = filepathSimulationOutput + "Test.root";
...
int main(int argc, char *argv[])
{
    ...
    std::string outputName = "/Users/idarosenqvist/Desktop/Academics/Codes/AlphaTherapyMaster/Analysis/AnalysisCode/OutputDNA";
